...
env_path = Path(__file__).resolve().parent.parent / 'Discord' / '.env'
load_dotenv(dotenv_path=env_path)

# Load the environment variables
token = os.getenv('TOKEN')
disc = DiscordWebhook(url=os.getenv('trafic_webhook'))
disc2 = DiscordWebhook(url=os.getenv('hospital_webhook'))
print(f"Traffic Webhook URL: {disc.url}")
print(f"Hospital Webhook URL: {disc2.url}")
...
def home():
    if request.method == "POST":
        ...
        if len(result) != 0:
            ...
            return render_template('/templates/index.html', password="False")
        db.commit()
...
def ambulancepage():
    ...
    if request.method == "POST":
        ...
        # Setting content for DiscordWebhook for hospital
        disc2.content = f"A patient is coming to {route_to}. Your hospital {text_hospital_name} cover in this location, get ready with the bed"
        disc2.execute()
        ...
        for ki in range(len(signal_details)):
            ...
            discord_notification = str(f"Ambulance is coming in {signal_details[ki][0]}, {signal_details[ki][1]}. Free the road from {route_from} to {route_to}")
            disc.content = discord_notification
            disc.execute()
...
# disc2=DiscordWebhook(url=os.getenv('hospital_webhook'))
# # app=Flask(__name__)
# app = Flask(__name__, template_folder='python/templates')
...
#         # Prepare hospital names for the message
#         text_hospital_name = ', '.join([f"{h[0]}" for h in hospital_details if h[1] == 1])

#         # Notify hospital via webhook
#         disc2.content = f"A patient is coming to {route_to}. Your hospital {text_hospital_name} covers this location, get ready with the bed."
#         try:
#             disc2.execute()  # Send notification to hospital
#             print("Hospital notification sent.")
#         except Exception as e:
#             print(f"Error sending to hospital webhook: {e}")

#         # Send traffic signal notification
#         for signal in signal_details:
#             status = "Free" if signal[2] == 1 else "Busy"
#             discord_notification = f"Ambulance is coming in {signal[0]}, {signal[1]}. Free the road from {route_from} to {route_to}."
#             disc.content = discord_notification
#             try:
#                 disc.execute()  # Send notification to traffic police
#                 print("Traffic notification sent.")
#             except Exception as e:
#                 print(f"Error sending to traffic webhook: {e}")

#         # Convert signal_details for display
#         for ki in range(len(signal_details)):
#             signal_details[ki] = list(signal_details[ki])
#             signal_details[ki][2] = "Free" if signal_details[ki][2] == 1 else "Busy"

#         # Convert hospital details for display
#         for ki in range(len(hospital_details)):
#             hospital_details[ki] = list(hospital_details[ki])
#             hospital_details[ki][1] = "Yes" if hospital_details[ki][1] == 1 else "No"
